...
import { PLATFORM_NAMES, mapToStandardPlatformName, getPlatformColor } from '../constants/platforms';
...
export default function HistoricalSocialPosts() {
    ...
    try {
      ...
      if (socialMediaContent.LinkedIn) {
        Object.entries(socialMediaContent.LinkedIn).forEach(([key, post]: [string, any]) => {
          posts.push({
            id: `linkedin-${postId++}`,
            platform: PLATFORM_NAMES.LINKEDIN,
            type: 'post',
      ...
      if (socialMediaContent.InstagramFacebook) {
        Object.entries(socialMediaContent.InstagramFacebook).forEach(([key, post]: [string, any]) => {
          posts.push({
            id: `instagram-${postId++}`,
            platform: PLATFORM_NAMES.INSTAGRAM_FACEBOOK,
            type: 'post',
            caption: post.Caption || '',
            imagePrompt: post.ImagePrompt || '',
            originalData: post
      ...
      if (socialMediaContent.TikTokReels) {
        Object.entries(socialMediaContent.TikTokReels).forEach(([key, video]: [string, any]) => {
          posts.push({
            id: `tiktok-${postId++}`,
            platform: PLATFORM_NAMES.TIKTOK_REELS,
            type: 'video',
  ...
  // Use standardized platform color function from constants
                ...
                <div className="bg-gray-50 rounded-lg p-4">
                  ...
                  <div className="flex items-center gap-3 mt-3 text-xs text-gray-500">
                    <span className="inline-flex items-center gap-1">
                      {getPlatformIcon(PLATFORM_NAMES.LINKEDIN, 'w-3 h-3')} {PLATFORM_NAMES.LINKEDIN}
                    </span>
                    <span className="inline-flex items-center gap-1">
                      {getPlatformIcon(PLATFORM_NAMES.INSTAGRAM_FACEBOOK, 'w-3 h-3')} {PLATFORM_NAMES.INSTAGRAM_FACEBOOK}
                    </span>
                    <span className="inline-flex items-center gap-1">
                      {getPlatformIcon(PLATFORM_NAMES.TIKTOK_REELS, 'w-3 h-3')} {PLATFORM_NAMES.TIKTOK_REELS}
                    </span>
                  </div>
                </div>
              </div>
