      ...
      <div className="space-y-6">
        ...
        <div className="mt-8">
          ...
          <div className="relative">
            ...
            {/* GBP inline modals */}
            {showSocialMediaPages && socialMediaPlatform === 'gbp' && (
              <Card className="col-span-full">
                <CardHeader>
                  <div className="flex items-center justify-between">
                    <div>
                      <CardTitle>Google Business Profile Accounts</CardTitle>
                      <CardDescription>Select Google Business Profile locations to connect for social media posting</CardDescription>
                    </div>
                    <Button variant="ghost" size="sm" onClick={() => setShowSocialMediaPages(false)}>
                      <X className="w-4 h-4" />
                    </Button>
            ...
            {showManageModal && managePlatform === 'gbp' && (
              <Card className="col-span-full">
                <CardHeader>
                  <div className="flex items-center justify-between">
                    <div><CardTitle>Manage Google Business Profile Integrations</CardTitle><CardDescription>View and manage all connected locations</CardDescription></div>
                    <Button variant="ghost" size="sm" onClick={() => setShowManageModal(false)}><X className="w-4 h-4" /></Button>
                  </div>
                </CardHeader>
                <CardContent>
                  <div className="space-y-3">
                    {connectedSocialMediaAccounts.filter(a => a.platform === 'gbp' && !a.deleted).map((account) => {
                      const isExpired = account.isExpired || false;
                      const expireDate = account.expire ? new Date(account.expire) : null;
                      const daysUntilExpire = expireDate ? Math.ceil((expireDate.getTime() - Date.now()) / (1000 * 60 * 60 * 24)) : null;
                      return (
            ...
            <Card className={`hover:shadow-lg transition-shadow ${(showSocialMediaPages && socialMediaPlatform === 'pinterest') || (showManageModal && managePlatform === 'pinterest') ? 'col-span-full' : ''}`}>
              <CardContent className="pt-6">
                <div className="text-center space-y-4">
                  <div className="w-20 h-20 mx-auto bg-white rounded-lg flex items-center justify-center p-2">
                    <img 
            ...
            {showManageModal && managePlatform === 'pinterest' && (
              <Card className="col-span-full">
                <CardHeader>
                  <div className="flex items-center justify-between">
                    <div><CardTitle>Manage Pinterest Integrations</CardTitle><CardDescription>View and manage all connected accounts</CardDescription></div>
                    <Button variant="ghost" size="sm" onClick={() => setShowManageModal(false)}><X className="w-4 h-4" /></Button>
                  </div>
                </CardHeader>
                <CardContent>
            ...
            {/* Community inline modals */}
            {showSocialMediaPages && socialMediaPlatform === 'community' && (
              <Card className="col-span-full">
                <CardHeader>
                  <div className="flex items-center justify-between">
                    <div>
                      <CardTitle>Community Social Media Accounts</CardTitle>
                      <CardDescription>Select Community accounts to connect for social media posting</CardDescription>
                    </div>
                    <Button variant="ghost" size="sm" onClick={() => setShowSocialMediaPages(false)}>
                      <X className="w-4 h-4" />
                    </Button>
            ...
            {showManageModal && managePlatform === 'community' && (
              <Card className="col-span-full">
                <CardHeader>
                  <div className="flex items-center justify-between">
                    <div><CardTitle>Manage Community Integrations</CardTitle><CardDescription>View and manage all connected accounts</CardDescription></div>
                    <Button variant="ghost" size="sm" onClick={() => setShowManageModal(false)}><X className="w-4 h-4" /></Button>
                  </div>
                </CardHeader>
                <CardContent>
                  <div className="space-y-3">
                    {connectedSocialMediaAccounts.filter(a => a.platform === 'community' && !a.deleted).map((account) => {
                      const isExpired = account.isExpired || false;
                      const expireDate = account.expire ? new Date(account.expire) : null;
                      const daysUntilExpire = expireDate ? Math.ceil((expireDate.getTime() - Date.now()) / (1000 * 60 * 60 * 24)) : null;
                      return (
            ...
            {/* Bluesky inline modals */}
            {showSocialMediaPages && socialMediaPlatform === 'bluesky' && (
              <Card className="col-span-full">
                <CardHeader>
                  <div className="flex items-center justify-between">
                    <div>
                      <CardTitle>Bluesky Social Media Accounts</CardTitle>
                      <CardDescription>Select Bluesky accounts to connect for social media posting</CardDescription>
                    </div>
                    <Button variant="ghost" size="sm" onClick={() => setShowSocialMediaPages(false)}>
                      <X className="w-4 h-4" />
                    </Button>
            ...
            {showManageModal && managePlatform === 'bluesky' && (
              <Card className="col-span-full">
                <CardHeader>
                  <div className="flex items-center justify-between">
                    <div><CardTitle>Manage Bluesky Integrations</CardTitle><CardDescription>View and manage all connected accounts</CardDescription></div>
                    <Button variant="ghost" size="sm" onClick={() => setShowManageModal(false)}><X className="w-4 h-4" /></Button>
                  </div>
                </CardHeader>
                <CardContent>
                  <div className="space-y-3">
                    {connectedSocialMediaAccounts.filter(a => a.platform === 'bluesky' && !a.deleted).map((account) => {
                      const isExpired = account.isExpired || false;
                      const expireDate = account.expire ? new Date(account.expire) : null;
                      const daysUntilExpire = expireDate ? Math.ceil((expireDate.getTime() - Date.now()) / (1000 * 60 * 60 * 24)) : null;
                      return (
            ...
            {/* Social Media Pages Modal (for Facebook, Instagram, LinkedIn only) */}
            {showSocialMediaPages && !['threads', 'gbp', 'tiktok', 'youtube', 'pinterest', 'community', 'bluesky'].includes(socialMediaPlatform) && (
              <Card className="col-span-full">
            <CardHeader>
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <CardDescription>
                    {socialMediaPlatform === 'instagram' 
                      ...
                      : socialMediaPlatform === 'facebook'
                      ? 'Select Facebook pages to connect for social media posting'
                      : socialMediaPlatform === 'tiktok'
                      ? 'Select TikTok accounts to connect for social media posting'
                      : socialMediaPlatform === 'threads'
                      ? 'Select Threads accounts to connect for social media posting'
                      : socialMediaPlatform === 'gbp'
                      ? 'Select Google Business Profile locations to connect for social media posting'
                      : socialMediaPlatform === 'youtube'
                      ? 'Select YouTube channels to connect for social media posting'
                      : socialMediaPlatform === 'pinterest'
                      ? 'Select Pinterest accounts to connect for social media posting'
                      : socialMediaPlatform === 'community'
                      ? 'Select Community accounts to connect for social media posting'
                      : socialMediaPlatform === 'bluesky'
                      ? 'Select Bluesky accounts to connect for social media posting'
                      : 'Select accounts to connect for social media posting'
                    }
                  </CardDescription>
                </div>
                <Button
            ...
            {/* Manage Integrations Modal (for Facebook, Instagram, LinkedIn only) */}
            {showManageModal && !['threads', 'gbp', 'tiktok', 'youtube', 'pinterest', 'community', 'bluesky'].includes(managePlatform) && (
              <Card className="col-span-full">
            <CardHeader>
              <div className="flex items-center justify-between">
                <div>
                  <CardTitle>
                    Manage {managePlatform === 'facebook' ? 'Facebook' 
                      : managePlatform === 'instagram' ? 'Instagram' 
                      : managePlatform === 'linkedin' ? 'LinkedIn'
                      : managePlatform === 'tiktok' ? 'TikTok'
                      : managePlatform === 'threads' ? 'Threads'
                      : managePlatform === 'gbp' ? 'Google Business Profile'
                      : managePlatform === 'youtube' ? 'YouTube'
                      : managePlatform === 'pinterest' ? 'Pinterest'
                      : managePlatform === 'community' ? 'Community'
                      : managePlatform === 'bluesky' ? 'Bluesky'
                      : managePlatform.charAt(0).toUpperCase() + managePlatform.slice(1)} Integrations
                  </CardTitle>
                  <CardDescription>
                    View and manage all connected {managePlatform === 'facebook' ? 'pages' 
                      : managePlatform === 'instagram' ? 'accounts' 
                      : managePlatform === 'linkedin' ? 'profiles'
                      : managePlatform === 'youtube' ? 'channels'
                      : managePlatform === 'gbp' ? 'locations'
                      : 'accounts'}
                  </CardDescription>
                </div>
                <Button
